...
See all suppliers with their categories, rate, and status at a glance.
Filter by country (USA / Spain) to manage each market separately.
Filter by category to answer questions like "what active carriers do we have in Spain?".
Register a new supplier from a form.
Update a supplier's per-shipment rate and see the change reflected immediately.
Suspend or reactivate a supplier with a visible control in the row.
